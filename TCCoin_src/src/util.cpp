...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TCChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TCChain
    // Mac: ~/Library/Application Support/TCChain
    // Unix: ~/.TCChain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TCChain";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TCChain.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty TCChain.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "TCChaind.pid"));
